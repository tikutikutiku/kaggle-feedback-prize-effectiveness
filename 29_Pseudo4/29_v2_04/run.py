        ...
        for epoch in range(1, args.epochs+1):
            ...
            for i,data in enumerate(tk0):
                ...
                with torch.cuda.amp.autocast(enabled=(args.fp16=='true')):
                    ...
                    if (args.ema_decay != 1.0) and (i + 1 > args.ema_start_step) and ((i + 1) % args.ema_interval == 0):
                        # update model_ema
                        for param, param_ema in zip(model.parameters(), model_ema.parameters()):
                            param_ema.data.mul_(args.ema_decay).add_((1-args.ema_decay)*param.data)
                ...
                if args.eval_step!=-1 and (i+1)%args.eval_step==0:
                    ...
                    for i,data in enumerate(val_dataloader):
                        with torch.no_grad():
                            if args.ema_decay != 1.0:
                                outputs.append(model_ema.cuda().validation_step(data))
                            else:
                        ...
                        gc.collect()
                    if args.ema_decay != 1.0:
                        val_loss, val_score = model_ema.cuda().validation_epoch_end(outputs)
                    else:
...
#                         print('model (best score) saved')
            ...
            if args.ema_decay != 1.0:
                if args.mode=='pseudo':
                    torch.save(model_ema.state_dict(), opj(output_path,f'model_seed{args.seed}_fold{args.fold}_epoch{epoch}pseudo_ema.pth')) #save
                else:
                    torch.save(model_ema.state_dict(), opj(output_path,f'model_seed{args.seed}_fold{args.fold}_epoch{epoch}_ema.pth')) #save
            else:
                if args.mode=='pseudo':
            ...
            if args.ema_decay != 1.0:
                if args.mode=='pseudo':
                    model.load_state_dict(torch.load(opj(output_path,f'model_seed{args.seed}_fold{args.fold}_epoch{epoch}pseudo_ema.pth')))
                else:
                    model.load_state_dict(torch.load(opj(output_path,f'model_seed{args.seed}_fold{args.fold}_epoch{epoch}_ema.pth')))
            model.eval()
            outputs = []
