...
#         elif self.mt=='true' and self.training:
#             if self.msd=='true' and self.training:
...
#             else:
        ...
        elif self.loss in ['bce']: # for training with pseudo-label
            logits = self.forward_logits(**input_data)
    ...
    def training_step_mt(self, batch):
        '''for multitask with bce loss'''
        data = to_gpu(batch)
        input_data = {
            'input_ids':data['input_ids'],
            'attention_mask':data['attention_mask'],
        ...
        # get loss
        logits = self.forward_logits(**input_data)
        loss  = self.get_losses2(logits[:,0].reshape(-1,), data['Ineffective'].reshape(-1,)).mean() / 3
        loss += self.get_losses2(logits[:,1].reshape(-1,), data['Adequate'].reshape(-1,)).mean() / 3
        loss += self.get_losses2(logits[:,2].reshape(-1,), data['Effective'].reshape(-1,)).mean() / 3
        return loss
...
#         return scheduler
    ...
    def get_losses2(self, logits, label):
        loss = self.mt_loss_fn(logits, label)
        return loss
