...
def run(args, trn_df, val_df, pseudo_df=None):
    ...
    if True:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            tk0 = tqdm(zip(trn_dataloader,pseudo_dataloader), total=int(len(trn_dataloader)))
            optimizer.zero_grad()
            trn_preds = []
            trn_trues = []
            ...
            for i,(data,data_mt) in enumerate(tk0):
            #for i,(data,data_mt) in enumerate(zip(trn_dataloader, pseudo_dataloader)):
                model.train()
                ...
                with torch.cuda.amp.autocast(enabled=(args.fp16=='true')):
                    ...
                    if pseudo_df is not None:
                        loss_mt = model.training_step_mt(data_mt)
                        loss_mt = loss_mt * data_mt['loss_weight'].cuda()
                        loss += args.w_mt * loss_mt
