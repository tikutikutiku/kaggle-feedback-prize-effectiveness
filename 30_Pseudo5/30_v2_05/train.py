...
def parse_args():
    ...
    parser.add_argument("--num_labels_2", type=int, default=3, required=False)
    ...
    parser.add_argument("--mt_input_path", type=str, default='none', required=False)
    parser.add_argument("--mt_fold_path", type=str, default='none', required=False)
...
if __name__=='__main__':
    ...
    if args.mt_input_path!='none':
        pseudo_df = pd.read_csv(args.mt_input_path)
        print('mt pseudo_df.shape = ', pseudo_df.shape)
        pseudo_df['label'] = pseudo_df[['Ineffective','Adequate','Effective']].values.argmax(-1)
        # use partial data
        pseudo_fold_path = args.mt_fold_path
        pseudo_ids_list = joblib.load(opj(pseudo_fold_path,f'trn_ids_list.joblib'))
        pseudo_df = pseudo_df[pseudo_df['essay_id'].isin(pseudo_ids_list[args.fold])].reset_index(drop=True)
        print('mt pseudo_df.shape = ', pseudo_df.shape)
    else:
        pseudo_df = None
    
    from run import run
    run(args, trn_df, val_df, pseudo_df=pseudo_df)