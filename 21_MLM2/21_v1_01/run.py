...
def run(args, trn_df, val_df, pseudo_df=None):
    ...
    if True:
        ...
        # dataset
        trn_dataset = DatasetTrain(trn_df, tokenizer, aug=args.aug)
        ...
        import models_pretrain_discriminator
        model_pretraining = models_pretrain_discriminator.Model(args.model, 
                                                                tokenizer,
                                                                num_labels=1,
                                                                )
        if args.pretrain_path != 'none':
            model_pretraining.load_state_dict(torch.load(args.pretrain_path))
        ...
        for epoch in range(1, args.epochs+1):
            ...
            # dataset
            if args.sampling=='simple':
                trn_dataset = DatasetTrain(trn_df, tokenizer, aug=args.aug)
            elif args.sampling=='balanced':
                balanced_df = []
                for anchor in sorted(trn_df['anchor'].unique()):
                    balanced_df.append(trn_df[trn_df['anchor']==anchor].sample(med_anchor_num, replace=True))
                balanced_df = pd.concat(balanced_df).reset_index(drop=True)
                print('balanced_df.shape = ', balanced_df.shape)
                trn_dataset = DatasetTrain(balanced_df, tokenizer, aug=args.aug)
            elif args.sampling=='pertubed':
                pertubed_df = trn_df.copy()
                pertubed_df['score'] = trn_df['score'].values + 0.03*np.random.randn(len(trn_df))
                trn_dataset = DatasetTrain(pertubed_df, tokenizer, aug=args.aug)
            ...
            for i,data in enumerate(tk0):
                ...
                if True:
                    loss, score = model.training_step(data)
                ...
                if args.eval_step!=-1 and (i+1)%args.eval_step==0:
                    ...
                    print('\nepoch {:.0f}: trn_loss = {:.4f}, val_loss = {:.4f}, trn_score = {:.4f}, val_score = {:.4f}'.format(
                        epoch, trn_loss / ((counter * trn_dataloader.batch_size)), 
                        val_loss, trn_score / ((counter * trn_dataloader.batch_size)), val_score))
