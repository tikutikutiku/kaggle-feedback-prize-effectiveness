...
def parse_args():
    ...
    parser.add_argument("--ema_decay", type=float, default=1.0, required=False)
    parser.add_argument("--ema_interval", type=int, default=1, required=False)
    parser.add_argument("--ema_start_step", type=int, default=3000, required=False)
...
#     #val_df = train_df[train_df['essay_id'].isin(val_ids_list[args.fold][:60])].reset_index(drop=True)
    ...
    #unique_ids = trn_df['essay_id'].unique()
    #trn_df = trn_df[trn_df['essay_id'].isin(unique_ids[:30])].reset_index(drop=True)
    val_df = None
