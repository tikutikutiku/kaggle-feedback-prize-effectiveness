...
class TextSpanDetector(nn.Module):
    ...
    def forward_logits(self, data):
        ...
        L = data['input_ids'].size(1)
        if self.window_size==-100 or L<=self.window_size:
            x = self.transformer(input_ids=data['input_ids'],
            ...
            if (L - self.window_size) % self.inner_len > self.edge_len:
                ...
                segments += 1
            x = self.transformer(input_ids=data['input_ids'][:,:self.window_size],
                                 attention_mask=data['attention_mask'][:,:self.window_size]).last_hidden_state
            for i in range(1,segments+1):
                ...
                x_next = self.transformer(input_ids=data['input_ids'][:,start:end],
                                          attention_mask=data['attention_mask'][:,start:end]).last_hidden_state
                if i==segments:
